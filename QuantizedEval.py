...
for model in models:
    ...
    imageDirec = "evaluationImages/" + modelName
    # Load the floating point model
    floatModel = tf.keras.models.load_model('models/' + modelName)
    with vitis_quantize.quantize_scope():
        intModel = tf.keras.models.load_model('quantizedmodel/' + modelName + '_Quantized.h5')
    ...
    print("Model under evaluation: " + modelName)
    print("Evaluation Floating point model")
    floatModel.evaluate(eval_ds)
    print("Evaluation INT8 model")
    intModel.evaluate(eval_ds)
